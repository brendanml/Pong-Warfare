...
class GameState {
    ...
    void Update(Ball &ball) {
        ...
        DrawText(TextFormat("%d", abs(ball.speed_x)), GetScreenWidth()/2 +20, GetScreenHeight()-50, 25, BLACK);
        if(!versus_player) {
...
void CheckForCollisions(int screen_height,int screen_width) {
...
int main() {
    ...
    while(!WindowShouldClose()) {
        BeginDrawing();
        // game.CheckGameState(ball);
        // game.CheckPlayer2Input();
        // game.CheckDifficultyChange(ball);
        game.Update(ball);
        ball.Update();
        player.Update();
        cpu.Update(game.versus_player, ball.y);
        CheckForCollisions(screen_height,screen_width);
